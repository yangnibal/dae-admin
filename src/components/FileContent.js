...
const FileContent = ({name, subject, grade, group, update, remove, seeFiles, link}) => {
    return(
        <div className="filecontent-container">
            ...
            <a target="blank" className="filecontent-text" href={link.charAt(link.length - 1)==="f" ? `/web/viewer.html?file=${encodeURIComponent(link)}` : "https://docs.google.com/viewer?url=" + link}>자료 보기</a>
